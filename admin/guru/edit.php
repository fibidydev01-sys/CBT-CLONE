...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama) || empty($username)) {
        ...
        $duplicate = false;
        foreach (['admin', 'pengawas', 'siswa'] as $table) {
        ...
        $stmt = $conn->prepare("SELECT id FROM guru WHERE username = ? AND id != ?");
        ...
        if ($duplicate) {
            ...
            if (!empty($password)) {
                if (strlen($password) < 4) {
                    ...
                    $stmt = $conn->prepare("UPDATE guru SET nama=?, username=?, password=? WHERE id=?");
            ...
            if (empty($error)) {
                if (empty($password)) {
                    $stmt = $conn->prepare("UPDATE guru SET nama=?, username=? WHERE id=?");
                    $stmt->bind_param("ssi", $nama, $username, $id);
                }

                if ($stmt->execute()) {
                    redirect(base_url('admin/guru/index.php'), 'success', 'Data guru "' . $nama . '" berhasil diupdate.');
                } else {
...
<div class="content-with-sidebar">
    ...
    <div class="content-area">
        <div class="page-header">
            <div>
                <h4><i class="fas fa-edit me-2"></i>Edit Guru</h4>
                <nav>
                    <ol class="breadcrumb">
        ...
        <div class="row">
            <div class="col-lg-6">
                <div class="card">
                    <div class="card-header">Edit: <?= e($guru['nama']) ?></div>
                    <div class="card-body">
                        <?php if ($error): ?>
                        ...
                        <form method="POST">
                            <div class="mb-3">
                                ...
                                <input type="text" name="nama" class="form-control" required
                                       value="<?= e($_POST['nama'] ?? $guru['nama']) ?>">
                            </div>

                            <div class="mb-3">
                                <label class="form-label">Username <span class="text-danger">*</span></label>
                                <input type="text" name="username" class="form-control" required
                                       value="<?= e($_POST['username'] ?? $guru['username']) ?>">
                            </div>
            ...
            <div class="col-lg-6">
                <div class="card">
                    <div class="card-header">Info Guru</div>
                    <div class="card-body">
                        <table class="table table-borderless mb-0">
                            <tr><td class="fw-semibold" width="140">ID</td><td><?= $guru['id'] ?></td></tr>
                            <tr><td class="fw-semibold">Username</td><td><code><?= e($guru['username']) ?></code></td></tr>
                            <tr><td class="fw-semibold">Terdaftar</td><td class="small"><?= format_datetime($guru['created_at']) ?></td></tr>
                        </table>
                    </div>
                </div>
            </div>
