...
if ($ujian['status'] !== 'draft') {
    redirect(base_url('guru/ujian/index.php'), 'warning', 'Ujian yang sudah aktif/selesai tidak bisa diedit.');
}
...
$bank_list = $conn->query("SELECT bs.*,
    ...
    HAVING jumlah_soal > 0 ORDER BY bs.nama_bank ASC");
...
$soal_list = $conn->query("SELECT s.* FROM soal s
    ...
    ORDER BY s.nomor_soal ASC LIMIT {$ujian['jumlah_soal']}");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $tab === 'info') {
    ...
    if (empty($nama_ujian) || $bank_soal_id <= 0 || empty($tanggal_ujian) ||
        ...
        try {
            ...
            $conn->commit();
            redirect(base_url('guru/ujian/edit.php?id=' . $id . '&tab=kunci'), 'success', 'Ujian berhasil diupdate.');
        } catch (Exception $e) {
            $conn->rollback();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $tab === 'kunci') {
    ...
    try {
        ...
        $stmt->close();
        $conn->commit();
        redirect(base_url('guru/ujian/edit.php?id=' . $id . '&tab=kunci'), 'success', 'Kunci jawaban berhasil disimpan.');
    } catch (Exception $e) {
        $conn->rollback();
...
<div class="content-with-sidebar">
    ...
    <div class="content-area">
        ...
        <ul class="nav nav-tabs mb-3">
            <li class="nav-item">
                <a class="nav-link <?= $tab === 'info' ? 'active' : '' ?>" href="<?= base_url('guru/ujian/edit.php?id=' . $id . '&tab=info') ?>">
                    <i class="fas fa-info-circle me-1"></i>Info Ujian</a>
            </li>
            <li class="nav-item">
                <a class="nav-link <?= $tab === 'kunci' ? 'active' : '' ?>" href="<?= base_url('guru/ujian/edit.php?id=' . $id . '&tab=kunci') ?>">
                    <i class="fas fa-key me-1"></i>Kunci Jawaban</a>
            </li>
        </ul>
            ...
            <div class="row">
                    ...
                    <div class="card mb-3">
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                ...
                                <input type="text" name="nama_ujian" class="form-control" required value="<?= e($_POST['nama_ujian'] ?? $ujian['nama_ujian']) ?>">
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="row">
                                <div class="col-md-4 mb-3"><label class="form-label">Tanggal</label><input type="date" name="tanggal_ujian" class="form-control" required value="<?= e($_POST['tanggal_ujian'] ?? $ujian['tanggal_ujian']) ?>"></div>
                                <div class="col-md-4 mb-3"><label class="form-label">Jam Mulai</label><input type="time" name="jam_mulai" class="form-control" required value="<?= e($_POST['jam_mulai'] ?? substr($ujian['jam_mulai'], 0, 5)) ?>"></div>
                                <div class="col-md-4 mb-3"><label class="form-label">Jam Selesai</label><input type="time" name="jam_selesai" class="form-control" required value="<?= e($_POST['jam_selesai'] ?? substr($ujian['jam_selesai'], 0, 5)) ?>"></div>
                            </div>
                            <div class="row">
                                <div class="col-md-6 mb-3"><label class="form-label">Jumlah Soal</label><input type="number" name="jumlah_soal" id="jumlahSoal" class="form-control" min="1" required value="<?= e($_POST['jumlah_soal'] ?? $ujian['jumlah_soal']) ?>"><div class="form-text" id="soalInfo"></div></div>
                                <div class="col-md-6 mb-3"><label class="form-label">Alokasi Waktu (menit)</label><input type="number" name="alokasi_waktu" class="form-control" min="5" required value="<?= e($_POST['alokasi_waktu'] ?? $ujian['alokasi_waktu']) ?>"></div>
                            </div>
                            <div class="row">
                                <div class="col-md-6"><div class="form-check form-switch"><input class="form-check-input" type="checkbox" name="acak_soal" <?= ($_POST['acak_soal'] ?? $ujian['acak_soal']) ? 'checked' : '' ?>><label class="form-check-label">Acak urutan soal</label></div></div>
                                <div class="col-md-6"><div class="form-check form-switch"><input class="form-check-input" type="checkbox" name="acak_jawaban" <?= ($_POST['acak_jawaban'] ?? $ujian['acak_jawaban']) ? 'checked' : '' ?>><label class="form-check-label">Acak urutan jawaban</label></div></div>
                            </div>
                        </div>
                    </div>
                    <div class="card mb-3">
                        <div class="card-header">Kelas Peserta <button type="button" class="btn btn-sm btn-outline-primary float-end" onclick="toggleAllKelas()">Pilih Semua</button></div>
                        <div class="card-body">
                            <div class="row">
                                <?php while ($k = $kelas_list->fetch_assoc()): ?>
                                <div class="col-md-4 mb-2">
                                    <div class="form-check">
                                        <input class="form-check-input kelas-cb" type="checkbox" name="kelas_ids[]" value="<?= $k['id'] ?>" id="kelas<?= $k['id'] ?>" <?= in_array($k['id'], $_POST['kelas_ids'] ?? $selected_kelas) ? 'checked' : '' ?>>
                                        <label class="form-check-label" for="kelas<?= $k['id'] ?>"><?= e($k['nama_kelas']) ?> <small class="text-muted">(<?= $k['jumlah_siswa'] ?>)</small></label>
                                    </div>
                                </div>
                                <?php endwhile; ?>
                            </div>
                ...
                <div class="col-lg-4">
                    <div class="d-grid gap-2">
                        <button type="submit" class="btn btn-primary btn-lg"><i class="fas fa-save me-1"></i>Update Info</button>
                        <a href="<?= base_url('guru/ujian/index.php') ?>" class="btn btn-secondary"><i class="fas fa-arrow-left me-1"></i>Kembali</a>
                    </div>
                </div>
            </div>
        </form>
                        ...
                        <div class="card-body p-0">
                            <div class="table-responsive">
                                <table class="table table-hover mb-0">
                                    <thead><tr><th width="60">No</th><th>Pertanyaan</th><th width="250">Kunci Jawaban</th><th width="80">Skor</th></tr></thead>
                                    <tbody>
                                        <?php $soal_list->data_seek(0); $idx = 0; while ($s = $soal_list->fetch_assoc()): ?>
                                        <tr>
                                            <td><span class="badge bg-secondary"><?= $s['nomor_soal'] ?></span><input type="hidden" name="nomor_soal[<?= $idx ?>]" value="<?= $s['nomor_soal'] ?>"></td>
                                            <td>
                                                <div class="small" style="max-width:400px;"><?= e(mb_substr(strip_tags($s['pertanyaan']), 0, 100)) ?><?= mb_strlen($s['pertanyaan']) > 100 ? '...' : '' ?></div>
                                                <?php if ($s['tipe_soal'] === 'PG' && isset($soal_opsi[$s['id']])): ?>
                                                <div class="mt-1"><?php foreach ($soal_opsi[$s['id']] as $key => $text): ?><small class="d-block text-muted"><strong><?= $key ?>.</strong> <?= e(mb_substr($text, 0, 50)) ?></small><?php endforeach; ?></div>
                                                <?php endif; ?>
                                            </td>
                                            <td>
                                                ...
                                                <div class="d-flex gap-1">
                                                    ...
                                                    <label class="btn btn-sm <?= $current_kunci === $key ? 'btn-primary' : 'btn-outline-secondary' ?> kunci-btn">
                                                        <input type="radio" name="jawaban_benar[<?= $idx ?>]" value="<?= $key ?>" <?= $current_kunci === $key ? 'checked' : '' ?> style="display:none;" onchange="highlightKunci(this)">
                                                        <?= $key ?>
                                                    </label>
                                                    <?php endforeach; ?>
                                                </div>
                                                <?php else: ?>
                                                <input type="text" name="jawaban_benar[<?= $idx ?>]" class="form-control form-control-sm" placeholder="Jawaban esai" value="<?= e($existing_kunci[$s['nomor_soal']]['jawaban_benar'] ?? '') ?>">
                                                <?php endif; ?>
                                            </td>
                                            <td><input type="number" name="skor[<?= $idx ?>]" class="form-control form-control-sm" step="0.5" min="0" value="<?= e($existing_kunci[$s['nomor_soal']]['skor'] ?? '1') ?>"></td>
                                        </tr>
                                        <?php $idx++; endwhile; ?>
                                    </tbody>
                                </table>
                ...
                <div class="col-lg-3">
                    <div class="card mb-3">
                        ...
                        <div class="card-body">
                            ...
                            <div class="alert alert-warning small mb-0"><i class="fas fa-exclamation-triangle me-1"></i>Lengkapi semua kunci jawaban sebelum mengaktifkan.</div>
                            <?php else: ?>
                            <div class="alert alert-success small mb-0"><i class="fas fa-check-circle me-1"></i>Semua kunci jawaban sudah terisi.</div>
                            <?php endif; ?>
                        </div>
                    </div>
                    <div class="d-grid gap-2">
                        <button type="submit" class="btn btn-primary btn-lg"><i class="fas fa-save me-1"></i>Simpan Kunci</button>
                        <?php if (count($existing_kunci) >= $ujian['jumlah_soal']): ?>
                        <a href="<?= base_url('guru/ujian/aktifkan.php?id=' . $id) ?>" class="btn btn-success" data-confirm="Aktifkan ujian ini?"><i class="fas fa-play me-1"></i>Aktifkan Ujian</a>
                        <?php endif; ?>
                        <a href="<?= base_url('guru/ujian/index.php') ?>" class="btn btn-secondary"><i class="fas fa-arrow-left me-1"></i>Kembali</a>
                    </div>
                </div>
